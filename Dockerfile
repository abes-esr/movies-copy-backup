...
RUN echo "3    0       *       *       *       docker exec  movies-db-dumper sh -c 'restore "'$(ls -t /backup/*.gz | head -n1) $DB_TYPE $DB_HOST $DB_NAME $DB_USER $DB_PASS 3306'"' >/dev/null 2>&1" >> /var/spool/cron/crontabs/root
